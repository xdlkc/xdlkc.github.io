...
test('initReadingProgress shows remaining minutes when data-reading-minutes exists', () => {
  ...
  assert.equal(label.textContent.trim(), '50% · 剩余 5 分钟');

  const container = document.querySelector('.reading-progress');
  assert.equal(container.getAttribute('aria-valuenow'), '50');
  assert.equal(container.getAttribute('aria-valuetext'), '阅读进度 50%，剩余 5 分钟');
